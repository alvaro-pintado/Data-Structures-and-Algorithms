...
//  Very simple initial testing...
	...
	for (int i=0; i<len0; i++) {
		if (!tstHash0.insert(words0[i], words0[i], words0[i])) {
	...
	for (int i=0; i<len0; i++) {
		if (!tstHash0.lookup(words0[i], tmp1, tmp2)) {
			cout << "Error, hash lost word " <<
				words0[i] << "." << endl;
		}
	}

	cout << endl << "Hash Dump (for testing)" << endl << bars << endl;
	tstHash0.printHash();
	cout << endl << endl;

	cout << "Test Hash Zero" << endl << endl;
...
//  Slightly larger test set.
	...
	for (int i=0; i<len1; i++) {
		if (!tstHash1.insert(words1[i], words1[i], words1[i])) {
			cout << "Error, unable to insert word " <<
				words1[i] << "." << endl;
		}
	}
	...
	for (int i=0; i<len1; i++) {
		if (!tstHash1.lookup(words1[i], tmp1, tmp2)) {
			cout << "Error, hash lost word " <<
				words1[i] << "." << endl;
		}
	}
...
//	'words' which are just unique letter strings for testing.
		...
		for (int j=0; j < 26; j++) {
			...
			for (int k=0; k < 26; k++) {
				...
				for (int l=0; l < 26; l++) {
					...
					str.append(1, ltr3);
					str.append(1, ltr4);
					if (!tstHash2.insert(str, str, str))
	...
	cout << "Test Hash Two A" << endl << endl;
	tstHash2.showHashStats();
	cout << endl;
//	tstHash2.printHash();		// testing only...
		...
		for (int j=0; j < 26; j++) {
			...
			for (int k=0; k < 26; k++) {
				...
				for (int l=0; l < 26; l++) {
					...
					str.append(1, ltr3);
					str.append(1, ltr4);
					if(!tstHash2.lookup(str, tmp1, tmp2))
						cout << "Error, hash lost word - "
							<< str << "." << endl;
					ltr4++;
				}
